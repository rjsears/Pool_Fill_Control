// This sketch modified by Richard Sears (richard@sears.net) to work with my automatic pool filling system.
// It has been updated to change the LED colors (my external push button only has a single LED color) and
...
#define NETWORKID     250  //all nodes on the same network can talk to each other
...
// Redefine LED Colors for use with single color (green) external push button
...
void handleSerialInput() {
  ...
  if (inputLen > 0)
  {
    ...
    if (inputstr.equals("PFC_START_OK")) { sprintf(lcdbuff, "Pool Fill Control\nStart OK!"); refreshLCD(); saveToHistory(lcdbuff, rssi); }
    if (inputstr.equals("PFC_AUTO_FILL")) { sprintf(lcdbuff, "Pool is auto\nFILLING!"); refreshLCD(); saveToHistory(lcdbuff, rssi); }
    if (inputstr.equals("PFC_MAN_FILL")) { sprintf(lcdbuff, "Pool is manual\nFILLING!"); refreshLCD(); saveToHistory(lcdbuff, rssi); }
    if (inputstr.equals("PFC_LEVEL_OK")) { sprintf(lcdbuff, "Pool level is\nOK!"); refreshLCD(); saveToHistory(lcdbuff, rssi); }
    if (inputstr.equals("PFC_LEVEL_LOW")) { sprintf(lcdbuff, "Pool level is\nLOW!"); refreshLCD(); saveToHistory(lcdbuff, rssi); }
    if (inputstr.equals("PFC_FILL_DONE")) { sprintf(lcdbuff, "Pool is DONE\nfilling!"); refreshLCD(); saveToHistory(lcdbuff, rssi); }
